"""
Премиум эмодзи — таблица и хелперы.
"""
import state
...
def by_line() -> str:
    star = pe("star_pe")
    return f'<i>by @B1nnary {star}</i>'


def toggle_pe(active: bool) -> str:
    """Анимированный вкл/выкл (как в ENG режиме)."""
    if active:
...
def status_pe(active: bool) -> str:
    """Анимированный статус — использует те же рабочие ID что и toggle_pe."""
