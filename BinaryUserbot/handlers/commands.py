...
from scam import check_scam_base, cmd_scam, cmd_lol, get_scam_identifiers
...
from antivirus import handle_antivirus_incoming
from prank import run_prank
from animations import run_animation, CAT_FRAMES
from help_faq import get_help_text, cmd_setting, FAQ_DATA, FAQ_KNOWLEDGE_BASE
from handlers.command_normalizer import normalize_command_text


EBALAJ_COMMANDS = {".ебалай", ".ebalaj", ".ebalay", ".ебалй"}
TROLL_COMMANDS = {".troll", ".троль", ".тролль", ".trol"}
...
async def on_new_message(event):
    ...
    if priv:
        ...
        if not msg.out and isinstance(sender, User) and sender.id not in state.known_users:
            ...
            identifiers = get_scam_identifiers(sender)
            if identifiers and await check_scam_base(*identifiers):
                shown = next((item for item in identifiers if not item.isdigit()), str(sender.id))
                await send_me(
                    f"🛑 <b>Binary Userbot обнаружил юзера</b> <code>{html(shown)}</code> <b>в скам базе.</b>"
                )

        if not msg.out and await handle_antivirus_incoming(event):
            return
    ...
    if not msg.out:
        ...
        if priv and msg.chat_id in state.ebalaj_active:
            await handle_ebalaj(event)
            return
        if priv and msg.chat_id in state.troll_active:
            await handle_troll(event)
            return
        if priv and state.ac_active.get(msg.chat_id):
            await handle_ac(event)
            return

        return

    if msg.text and not priv:
        from manager_bot import handle_pending_reply

        if await handle_pending_reply(
            msg.chat_id,
    ...
    raw = normalize_command_text(msg.text.strip()).strip()
    cmd = raw.lower()
    ...
    if cmd == ".help":
        from manager_bot import send_help_via_manager

        sent = await send_help_via_manager(event.chat_id, get_help_text())
        if sent:
            try:
                await event.message.delete()
            except Exception:
        ...
        from manager_bot import cmd_sb

        await cmd_sb(event)

    elif cmd == ".sbc":
        from manager_bot import cmd_sbc

        await cmd_sbc(event)

    elif cmd.startswith(".sbt ") or cmd == ".sbt":
        from manager_bot import cmd_sbt

        await cmd_sbt(event, raw[4:].strip())

    elif cmd == ".restart":
        from manager_bot import restarting_text, schedule_restart, post_restart_notice_for_user_message
        ...
        await event.message.edit(
            f"{pe('alien')} <b>BinaryUserbot</b>\n\n"
            "<blockquote>"
            "BinaryUserbot - юзербот с огромным функционалом, имеющий открытый исходный код, "
            "который каждый наш юзер может изменить под себя. Проект полностью был разработан "
            "@burgerbeats, наша студия разработки: @bincore. Мы создаём это для обычных пользователей, "
            "который хотят бесплатно добавить огромное количество фич в свой телеграм. Да, много модулей "
            "взято из других юзерботов - но мы и не выдаём их за свои! Наш юзербот - уникальная вещь, "
            "которая писалась с 0. Вы здесь можете кастомизировать всё и создавать свои модули даже без "
            "знания языков програмирования. Желаем приятного пользования)"
            "</blockquote>\n\n"
            "<blockquote>"
            "Также хотим напомнить, что во благо вашей безопасности советуем никогда не запускать "
            "незнакомые программы и вводить неизвестные команды через <code>.terminal</code>, "
            "из-за несоблюдения этого правила вы можете лишиться своего Telegram аккаунта."
            "</blockquote>\n\n"
            f"{pe('chain')} GitHub: <a href=\"https://github.com/binary166/BinaryUserbot\">binary166/BinaryUserbot</a>\n\n"
            f"{by_line()}",
            parse_mode="html",
            link_preview=False,
        )
    ...
    elif re.match(r"^\.calc(?:\s+.+)?$", cmd):
        expr = raw[5:].strip()
        if not expr:
            await event.message.edit(
                '<tg-emoji emoji-id="5935938364086685805">🧮</tg-emoji> <b>Калькулятор</b>\n\n'
                '<b>Использование:</b>\n<code>.calc 2+2</code>\n<code>.calc 100 тон в рубли</code>',
                parse_mode="html",
            )
            return
        ...
        if crypto_result:
            ...
            calc_emoji = '<tg-emoji emoji-id="5935938364086685805">🧮</tg-emoji>'
            await event.message.edit(
                f"{calc_emoji} <b>Калькулятор</b>\n\n"
                f"<b>Выражение:</b>\n<code>{html(expr)}</code>\n\n"
        ...
        rest_raw = raw[4:].strip()
        rest = rest_raw.lower().strip()
        try:
        ...
        if not rest:
            ...
            if info:
                ...
                question = rest_raw.strip().strip('"').strip("'").strip()
                command_lines = []
                for faq_cmd, faq_text in list(combined_faq.items())[:120]:
                    plain = re.sub(r"<[^>]+>", "", faq_text)
                    plain = re.sub(r"\s+", " ", plain).strip()
                    command_lines.append(f"{faq_cmd}: {plain[:180]}")

                await event.message.edit(LOADING, parse_mode="html")
                try:
                    answer = await or_request(
                        (
                            "Ты FAQ-помощник Binary Userbot. Отвечай на русском, коротко и по делу. "
                            "Используй только базу знаний и список команд ниже. "
                            "Если точной информации нет, честно скажи, что не уверен, и предложи посмотреть .help или .faq .команда. "
                            "Можно использовать HTML-теги <b>, <i>, <code>, <blockquote>. Не используй markdown."
                        ),
                        (
                            f"База знаний:\n{FAQ_KNOWLEDGE_BASE}\n\n"
                            f"Команды:\n" + "\n".join(command_lines) + "\n\n"
                    ...
                    await event.message.edit(
                        f"❌ <b>FAQ AI ошибка:</b> <code>{html(str(e)[:250])}</code>\n\n"
        ...
        if not rest:
            await event.message.edit(
                ...
                f"🔒 Чат: <code>{state.bw_chat_id if state.bw_chat_id else 'не задан'}</code>\n\n"
                f"<b>Управление:</b>\n"
                f"<code>.bw слово</code> — добавить\n"
                f"<code>.bw список</code> — показать все\n"
                f"<code>.bw очистить</code> — удалить все\n"
                f"<code>.bwchat @чат</code> — установить чат\n\n" + by_line(),
                parse_mode="html"
            )
    ...
    elif cmd in EBALAJ_COMMANDS:
        if not priv:
            await event.message.edit(
                "❗ Только в <b>личных чатах</b>.", parse_mode="html"
            )
            return
        state.troll_active.pop(msg.chat_id, None)
        state.troll_history.pop(msg.chat_id, None)
        state.ebalaj_active[msg.chat_id]  = 0
        state.ebalaj_history[msg.chat_id] = [{
            "role": "system",
            "content": settings.get("ebalaj_system_prompt") or EBALAJ_SYSTEM,
        }]
        await event.message.edit("🤪 <b>Ебалай включён.</b>\nБуду тупить на каждое сообщение.", parse_mode="html")
    ...
    elif cmd in TROLL_COMMANDS:
        if not priv:
            await event.message.edit(
                "❗ Только в <b>личных чатах</b>.", parse_mode="html"
            )
            return
        state.ebalaj_active.pop(msg.chat_id, None)
        state.ebalaj_history.pop(msg.chat_id, None)
        state.troll_active[msg.chat_id]  = 0
        state.troll_history[msg.chat_id] = [{
            "role": "system",
            "content": settings.get("troll_system_prompt") or TROLL_SYSTEM,
        }]
        await event.message.edit("😡 <b>Тролль включён.</b>\nБуду злиться и отвечать жёстко.", parse_mode="html")
        await asyncio.sleep(0.8)
        try:
            await event.message.delete()
        except Exception:
        ...
        try:
            proc = await spawn_terminal_process(terminal_cmd, cwd=os.getcwd())
            stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=30)
            out    = decode_terminal_output(stdout)
            err    = decode_terminal_output(stderr)
