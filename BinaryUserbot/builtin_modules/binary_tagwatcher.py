...
K_TAGGER_ENABLED = "tagwatcher_enabled"
K_PM_AUTOREAD = "tagwatcher_pm_autoread"
K_PM_MARK_UNREAD = "tagwatcher_pm_mark_unread"
K_IGNORE_BOTS = "tagwatcher_ignore_bots"


def _ensure_defaults():
    if not hasattr(state, "tagger_enabled"):
        state.tagger_enabled = bool(settings.get(K_TAGGER_ENABLED, True))
    if not hasattr(state, "pm_autoread"):
        state.pm_autoread = bool(settings.get(K_PM_AUTOREAD, False))
    if not hasattr(state, "ignore_bots"):
        state.ignore_bots = bool(settings.get(K_IGNORE_BOTS, True))
    ...
    if not hasattr(state, "pm_mark_unread"):
        state.pm_mark_unread = bool(settings.get(K_PM_MARK_UNREAD, False))
...
async def _status_text() -> str:
    return (
        ...
        f"Куда приходят уведомления: <code>{html(str(state.logs_chat_id))}</code>\n\n"
...
@client.on(events.NewMessage(pattern=r"^\.\s*tagwatcher(?:\s+(.+))?$", outgoing=True))
async def tagwatcher_cmd(event):
    ...
    if arg in {"pm", "лс", "autoread"}:
        ...
        settings.set_val(K_PM_AUTOREAD, bool(state.pm_autoread))
    ...
    if arg in {"unread", "непрочит", "mark"}:
        ...
        settings.set_val(K_PM_MARK_UNREAD, bool(state.pm_mark_unread))
        return await event.edit(
    ...
    settings.set_val(K_TAGGER_ENABLED, bool(state.tagger_enabled))
    await event.edit(
