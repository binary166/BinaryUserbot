...
class YaMusicMod(loader.Module):
    ...
    strings = {
        ...
        "_cfg": {
            ...
            "now_playing_text": "Текст сообщения с текущим треком",
            "autobio_text": "Текст автобио, когда музыка играет",
            "no_playing_bio_text": "Текст автобио, когда музыка не играет",
            "banner_version": "Версия баннера",
        },
        "iguide": (
            "<b>YaMusic</b>\n\n"
            "Нужен OAuth-токен Яндекс.Музыки. Укажи его в конфиге модуля/настройках, "
            "после этого команды .yq, .yn, .ynt, .ylike и другие начнут работать."
        ),
        "errors": {
            "no_token_or_invalid": "❌ Токен Яндекс.Музыки не указан или недействителен.",
            "no_query": "❗ Укажи запрос: <code>.yq название трека</code>",
            "not_found": "❌ Трек не найден.",
            "no_playing": "❌ Сейчас ничего не играет.",
            "error": "❌ Не удалось получить данные трека.",
        },
        "autobio": {
            "enabled": "✅ Автобио YaMusic включено.",
            "disabled": "✅ Автобио YaMusic выключено.",
        },
        "search": (
            "🎧 <b>{performer} — {title}</b>\n\n"
        ...
        "downloading_track": "\n<i>Скачиваю трек...</i>",
        "uploading_banner": "🎧 <b>Готовлю баннер текущего трека...</b>",
        "_entity_types": {
            "PLAYLIST": "плейлист {}",
            "ALBUM": "альбом {}",
            "ARTIST": "артист {}",
        ...
        "likes": {
            "liked": "❤️ Лайкнул <a href=\"https://music.yandex.ru/track/{track_id}\">{track}</a>",
            "unliked": "💔 Убрал лайк с <a href=\"https://music.yandex.ru/track/{track_id}\">{track}</a>",
            "disliked": "👎 Дизлайкнул <a href=\"https://music.yandex.ru/track/{track_id}\">{track}</a>",
        },
        "lyrics": (
        ...
        "no_lyrics": "❌ Текст для <a href=\"https://music.yandex.ru/track/{track_id}\">{track}</a> не найден.",
    }
    ...
    async def client_ready(self, client, db):
        ...
        saved_token = self.get("token")
        if saved_token and not self.config["token"]:
    ...
    @loader.command(ru_doc="👉 Установить OAuth-токен Яндекс.Музыки", alias="ytoken")
    async def ymtokencmd(self, message: telethon.types.Message):
        """Set Yandex.Music OAuth token"""
        ...
        if not arg:
            return await utils.answer(
                message,
                "<b>YaMusic token</b>\n\n"
                f"Текущий токен: <code>{utils.escape_html(self._mask_token(self.config['token']))}</code>\n\n"
                "<code>.ymtoken TOKEN</code> — сохранить токен\n"
                "<code>.ymtoken clear</code> — удалить токен",
            )

        if arg.lower() in {"clear", "reset", "delete", "del", "очистить", "сброс"}:
            self.config["token"] = None
            self.ym_client = None
            self.set("token", None)
            return await utils.answer(
                message,
                "✅ <b>Токен YaMusic удалён.</b>\n\n"
                "<code>.ymtoken TOKEN</code> — установить новый токен",
            )

        self.config["token"] = arg
        self.ym_client = None
        self.set("token", arg)

        client = await self._get_ym_client()
        if client:
            status = "✅ <b>Токен YaMusic сохранён и проверен.</b>"
        else:
            status = "⚠️ <b>Токен YaMusic сохранён, но проверить его не удалось.</b>"

        await utils.answer(
            message,
            f"{status}\n\n"
            f"Токен: <code>{utils.escape_html(self._mask_token(arg))}</code>\n"
            "Теперь можно использовать <code>.yq</code>, <code>.yn</code>, <code>.ynt</code> и другие команды.",
        )
