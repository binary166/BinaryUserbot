...
async def get_last_news() -> str:
    ...
    try:
        summary = await or_request(
            ...
            "Пиши живо, без канцелярита. На русском. В конце каждой новости через пробел добавь эмодзи какой-нибудь. Не используй никакого форматировани!",
            f"Посты за 24 часа:\n\n{combined[:3000]}",
            max_tokens=700
        )
    except Exception as e:
    ...
    return (
        '<b><blockquote><tg-emoji emoji-id="5258328383183396223">📖</tg-emoji>  ДАЙДЖЕСТ НОВОСТЕЙ</blockquote></b>\n\n'
        + summary +
