...
import state
import settings
from bot_client import client as user_client
from config import (
...
BANNER_PATH = ASSETS_DIR / "баннер.png"
...
async def _inline_text_article(builder, title: str, description: str, text: str, buttons=None, prefix: str = "result"):
    ...
    print(f"[manager_bot inline] {prefix}: custom_emoji_entities={sum(1 for ent in (entities or []) if ent.__class__.__name__ == 'MessageEntityCustomEmoji')}")
...
def restarting_text() -> str:
    return f"<blockquote><b>{E_RESTART} Перезапускаю вашего юзербота...</b></blockquote>"
...
def update_available_text(latest_version: str) -> str:
    return (
...
async def _guard_event(event) -> bool:
    ...
    if not ok:
        try:
            await event.edit(subscription_required_text(), parse_mode="html", buttons=None, link_preview=False)
        except Exception:
...
async def _show_main(event) -> None:
    ...
    try:
        await event.edit(main_menu_text(), parse_mode="html", buttons=main_buttons(), link_preview=False)
    except Exception:
...
async def _show_modes(event) -> None:
    pending_inputs.pop(MY_ID, None)
    try:
        await event.edit(modes_menu_text(), parse_mode="html", buttons=modes_buttons(), link_preview=False)
    except Exception:
                ...
                if is_manager_running() and manager_client:
                    if BANNER_PATH.exists():
                        await manager_client.send_file(
                            MY_ID,
                            BANNER_PATH,
                            caption=update_available_text(latest),
                            parse_mode="html",
...
async def send_help_via_manager(chat_id: int, caption: str) -> bool:
    ...
    try:
        if BANNER_PATH.exists():
            await user_client.send_file(chat_id, BANNER_PATH, caption=caption, parse_mode="html", link_preview=False)
        else:
...
async def cmd_sbt(event, token: str) -> None:
    ...
    if not token:
        await event.message.edit(
            f"{E_LINK} Укажите токен: <code>.sbt 123456:ABC...</code>",
            parse_mode="html",
        )
        return
    ...
    settings.set_val("manager_bot_configured_username", None)
...
async def get_latest_version() -> str | None:
    ...
    match = re.search(r"v\s*\d+(?:\.\d+)+", text, flags=re.IGNORECASE)
...
async def _handle_check_updates(event) -> None:
    ...
    if not latest:
        ...
        return
    if _is_newer(latest):
        await event.edit(update_available_text(latest), parse_mode="html", buttons=update_buttons(), link_preview=False)
    else:
        await event.edit(up_to_date_text(), parse_mode="html", buttons=back_buttons(), link_preview=False)
...
def _preserved_update_paths() -> list[Path]:
    patterns = ("config.py", "settings.json", "notes.json", "*.session", "*.session-journal")
    paths: list[Path] = []
    seen: set[str] = set()
...
def _skip_update_path(rel: Path) -> bool:
    parts = set(rel.parts)
    name = rel.name
    if parts & {"modules", "__pycache__", ".git", ".venv", "venv"}:
        return True
...
async def _handle_callback(event) -> None:
    ...
    if data == "back_main":
        ...
        pending_inputs.pop(MY_ID, None)
        await event.edit(openrouter_menu_text(), parse_mode="html", buttons=openrouter_buttons(), link_preview=False)
        ...
        await event.edit(input_prompt_text("or_token"), parse_mode="html", buttons=back_buttons(), link_preview=False)
        ...
        await event.edit(input_prompt_text("or_model"), parse_mode="html", buttons=back_buttons(), link_preview=False)
    elif data == "modes":
        await _show_modes(event)
        ...
        await event.edit(input_prompt_text("mode_ebalaj_prompt"), parse_mode="html", buttons=mode_input_buttons(), link_preview=False)
        ...
        await event.edit(input_prompt_text("mode_troll_prompt"), parse_mode="html", buttons=mode_input_buttons(), link_preview=False)
        ...
        await event.edit(restart_question_text(), parse_mode="html", buttons=restart_confirm_buttons(), link_preview=False)
    elif data == "restart_yes":
        await event.edit(restarting_text(), parse_mode="html", buttons=None, link_preview=False)
        schedule_restart(1.0)
    elif data == "terminal":
        pending_inputs[MY_ID] = {"kind": "terminal", "chat_id": event.chat_id, "message_id": event.message_id, "seen_source_ids": set()}
        await event.edit(terminal_prompt_text(), parse_mode="html", buttons=back_buttons(), link_preview=False)
