...
def _remove_registered(path: Path) -> None:
    ...
    if client:
        # Remove explicitly saved handlers for compat modules
        for module_name in module_names:
            old = sys.modules.pop(module_name, None)
            if old and hasattr(old, "_binary_handlers"):
        ...
        # Remove native telethon handlers bound to the module
        builders = getattr(client, "_event_builders", [])
        if builders:
            new_builders = []
            for item in builders:
                try:
                    builder, callback = item
                    cb_mod = getattr(callback, "__module__", "")
                    
                    # Unpack partials or decorators if necessary to find the real module
                    if not cb_mod and hasattr(callback, "__wrapped__"):
                        cb_mod = getattr(callback.__wrapped__, "__module__", "")
                except Exception:
                    new_builders.append(item)
                    continue

                if cb_mod in module_names:
...
def _register_loader_module(mod, path: Path, meta: dict, builtin: bool, module_name: str) -> list[dict]:
    ...
    for obj in list(vars(mod).values()):
        ...
        if not compat_loader.inline_manager:
            compat_loader.inline_manager = compat_loader.InlineManager(client)
        instance.inline = compat_loader.inline_manager
        instance.db = getattr(compat_loader, "db_instance", None)
...
async def run_deferred_startups():
    ...
    for instance in list(_loader_instances):
        ...
        if not hasattr(instance, "inline"):
            ...
            instance.inline = loader.inline_manager
        if not hasattr(instance, "allmodules"):
            instance.allmodules = types.SimpleNamespace()
        try:
            db_to_pass = getattr(instance, "db", None)
            result = ready(client, db_to_pass)
            if inspect.isawaitable(result):
